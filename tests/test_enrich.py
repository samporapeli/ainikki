...
def test_enrich_candidates_mixed_outcomes():
    """3 candidates: 1 succeeds, 1 is behind a paywall, 1 returns HTTP 500 -
    verifies that only the successful one remains and both failures appear as warnings."""
    scored = [
    ...
    assert len(result.items) == 1, f"expected 1 successful, got {len(result.items)}"
    ...
    assert len(result.warnings) == 2, f"expected 2 warnings, got {len(result.warnings)}"
    assert any("Tilaajaseinän" in w for w in result.warnings)
    assert any("Palvelin kaatuu" in w for w in result.warnings)

    assert len(result.dropped_stories) == 2, f"expected 2 dropped stories, got {len(result.dropped_stories)}"
