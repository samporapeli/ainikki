...
def test_build_score_prompt_includes_rubric_content():
    ...
    assert "vähintään 9" in system_prompt  # 3 * max(3)
...
def test_score_clusters_happy_path():
    ...
    assert result.scored[0].items[0].title.startswith("Anthropic announces")
    assert result.scored[3].rank == 4
    assert "tutkimustulos" in result.scored[3].selection_reason
